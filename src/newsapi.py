...
# *** FIX: Import datetime class specifically, or keep 'import datetime' and use datetime.datetime consistently ***
import datetime # Keep this
from datetime import timedelta # Keep this if used elsewhere, otherwise remove
import pytz
import time
...
def get_news(stock, lang='en'):
    ...
    try:
        ...
        params = {
            ...
            'articleBodyLen': 250, 'dataType': ['news'], # Use list for dataType
            'forceMaxDataTimeWindow': 3,
            'lang': lang,
            'keyword': stock,
            'dateStart': (datetime.date.today() - datetime.timedelta(days=3)).strftime('%Y-%m-%d'), # Ensure dateStart is recent
    ...
    except requests.exceptions.Timeout: logger.error(f"Timeout while fetching news ({stock}) : {NEWSAPI_ENDPOINT}"); return {'error': 'Timeout while fetching news.'}
    except requests.exceptions.RequestException as e: logger.error(f"Network error while fetching news ({stock}): {str(e)}"); return {'error': f'Network error while fetching news: {e}'}
    except Exception as e: logger.error(f"General error while fetching news ({stock}): {str(e)}", exc_info=True); return {'error': f'An unexpected error occurred while fetching news.'}
...
def get_stock_data(stock):
    ...
    empty_result = {
        ...
        'timestamp': datetime.datetime.now(pytz.utc).strftime('%Y-%m-%dT%H:%M:%S.000Z'), # Fix 1
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                else: raise ValueError("No ticker information available from fast_info or info")
        ...
        for period in ['6mo', '1y', '3mo', '1mo']:
            try:
                ...
                else: logger.warning(f"Empty or no data returned for {period} {stock}")
            except Exception as e_hist: logger.error(f"Fetching {period} history failed for {stock}: {type(e_hist).__name__} - {e_hist}"); hist = None
        ...
        if hist is None or hist.empty:
            try:
                ...
                end_date = datetime.date.today(); start_date = end_date - datetime.timedelta(days=90) # Use date objects
                hist_download = yf.download(stock, start=start_date.strftime("%Y-%m-%d"), end=end_date.strftime("%Y-%m-%d"), progress=False, timeout=30)
                if hist_download is not None and not hist_download.empty: logger.info(f"Backup download successful for {stock}, got {len(hist_download)} rows"); hist = hist_download
        ...
        if hist is None or hist.empty:
            ...
            empty_result['company_name'] = info.get('shortName', info.get('longName', stock)); empty_result['market_status'] = market_status
            return empty_result
        ...
        try:
            ...
            for col in required_columns:
                if col not in hist.columns: logger.warning(f"Missing column {col} in data for {stock}, adding as NaN"); hist[col] = float('nan')
            ...
            if hist.empty: logger.error(f"Date conversion resulted in empty DataFrame for {stock}"); empty_result['error'] = f"Date data for '{stock}' could not be processed."; return empty_result
            ...
            logger.error(f"Error formatting dates for {stock}: {date_fmt_err}", exc_info=True); empty_result['error'] = f"Date data for '{stock}' could not be processed."; return empty_result
        ...
        try:
            ...
            if not any(v is not None for v in close_values): logger.error(f"No valid close prices found after processing for {stock}"); empty_result['error'] = f"No valid price data available for '{stock}'."; return empty_result
            logger.info(f"Successfully converted data types for {stock}")
        except Exception as conv_err:
            logger.error(f"Error converting OHLCV data types for {stock}: {conv_err}", exc_info=True); empty_result['error'] = f"Error during data conversion for '{stock}'."; return empty_result
        ...
        if last_valid_close_index != -1:
            ...
            current_price = close_values[actual_index]; prev_close_hist = None
            ...
            try:
                 ...
                 if prev_close_info is not None and prev_close_info != 0: change_percent = ((current_price - prev_close_info) / prev_close_info * 100); logger.debug(f"Used info.previousClose ({prev_close_info}) for {stock} change %")
                 elif fast_info and fast_info.get('previous_close') is not None and fast_info['previous_close'] != 0: prev_close_fast = fast_info['previous_close']; change_percent = ((current_price - prev_close_fast) / prev_close_fast * 100); logger.debug(f"Used fast_info.previous_close ({prev_close_fast}) for {stock} change %")
                 elif prev_close_hist is not None and prev_close_hist != 0: change_percent = ((current_price - prev_close_hist) / prev_close_hist * 100); logger.debug(f"Used historical previous close ({prev_close_hist}) for {stock} change %")
                 else: logger.warning(f"Could not determine a valid previous close price for {stock} from info, fast_info, or history.")
            except Exception as prev_err:
                 logger.warning(f"Error accessing previous close price for {stock}: {prev_err}. Falling back to historical if available.")
                 if prev_close_hist is not None and prev_close_hist != 0: change_percent = ((current_price - prev_close_hist) / prev_close_hist * 100); logger.debug(f"Used historical previous close ({prev_close_hist}) for {stock} change % after error.")
                 else: logger.warning(f"Could not determine previous close after error for {stock}.")
        else:
            logger.warning(f"Could not find last valid close price in historical data for {stock}")
            current_price = info.get('regularMarketPrice', info.get('currentPrice'))
            prev_close_info = info.get('previousClose')
            if current_price is not None and prev_close_info is not None and prev_close_info != 0: change_percent = ((current_price - prev_close_info) / prev_close_info * 100); logger.debug(f"Used info.regularMarketPrice ({current_price}) and info.previousClose ({prev_close_info}) for {stock} change %")
        ...
        for i in range(len(labels)):
            if i < len(open_values) and i < len(high_values) and i < len(low_values) and i < len(close_values):
                ...
                if all(v is not None and pd.notna(v) for v in [o, h, l, c]): candlestick_data.append({'t': labels[i], 'o': o, 'h': h, 'l': l, 'c': c})

        company_name = info.get('shortName', info.get('longName', stock))
        currency = info.get('currency', 'USD')
        # *** FIX: Use datetime.datetime.now() ***
        timestamp_now = datetime.datetime.now(pytz.utc).strftime('%Y-%m-%dT%H:%M:%S.000Z') # Fix 2
        ...
        return {
            ...
            'timestamp': timestamp_now, # Use the fixed timestamp
            'error': None
        }
